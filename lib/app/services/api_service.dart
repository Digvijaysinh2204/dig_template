...
class ApiService extends GetxService {
  ...
  Future<Result<T>> request<T>({
    ...
    if (!Get.find<NetworkService>().isConnected.value) {
      ...
      return Result.error(Get.context!.loc.noConnection, null);
    }
  ...
  Map<String, dynamic> _prepareBody(dynamic body) {
    ...
    final encrypted = Get.find<CryptoService>().encryptGCM(
  ...
  String _extractErrorMessage(dynamic parsed, int statusCode) {
    ...
    if (parsed is String && parsed.isNotEmpty) {
      if (parsed.length < 200 && !parsed.contains('<html')) return parsed;
    }
